...
import { deleteFiles, downloadFile, pack, uploadData, uploadFile } from '../sync'
  ...
  return (
    <>
      <Head>
        ...
        {/* Disable pinch zooming but allow user scaling for better accessibility */}
        <meta
          name="viewport"
...
const Library: React.FC = () => {
  ...
  useEffect(() => {
    ...
    db?.books.toArray().then(async (books) => {
      for (const remoteFile of remoteFiles) {
        ...
        setLoading(book.id)
        await downloadFile(remoteFile.name).then((blob) => {
  ...
  return (
    ...
    >
                ...
                <Button
                  onClick={async () => {
                    ...
                    for (const book of selectedBooks) {
                      ...
                      setLoading(book.id)
                      await uploadFile(book.name, file.file)
                      setLoading(undefined)
      ...
      <div 
        className="overflow-y-auto flex-grow touch-pan-y"
        style={{ minHeight: 0 }} // Needed for flex child to be scrollable
      >
        <ul
          className="grid"
          style={{
